...
\tikzstyle{core} = [rectangle, rounded corners, minimum width=3cm, minimum height=1cm,text centered, draw=black, fill=red!10]
\tikzstyle{io} = [rectangle, rounded corners, minimum width=3cm, minimum height=1cm,text centered, draw=black, fill=green!10]
\tikzstyle{arrow} = [thick,->,>=stealth]

\node (core) [core] {MedImage\_data\_struct};
\node (io) [io, below of=core, xshift=-2.5cm] {Load\_and\_save};
\node (basic) [module, below of=core, xshift=2.5cm] {Basic\_transformations};
\node (spatial) [module, below of=io, xshift=2.5cm] {Spatial\_metadata\_change};
\node (ext) [module, below of=spatial] {External Tools (SimpleITK)};

\draw [arrow] (io) -- (core);
\draw [arrow] (basic) -- (core);
\draw [arrow] (spatial) -- (core);
\draw [arrow] (spatial) -- (ext);
\end{tikzpicture}
\caption{Architecture of MedImages.jl showing the core data structure and interacting modules.}
