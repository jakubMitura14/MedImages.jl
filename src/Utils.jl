...
@kernel function interpolate_kernel(out_res, @Const(source_arr_shape), @Const(source_arr), @Const(points_to_interpolate), @Const(spacing), keep_begining_same, extrapolate_value, is_nearest_neighbour)
    ...
    if (real_x < 1 || real_y < 1 || real_z < 1 || real_x > source_arr_shape[1] || real_y > source_arr_shape[2] || real_z > source_arr_shape[3])
        out_res[I] = extrapolate_value
    else
        if (real_x < 1 && keep_begining_same)
            real_x = 1.0f0
        end
        if (real_y < 1 && keep_begining_same)
            real_y = 1.0f0
        end
        if (real_z < 1 && keep_begining_same)
            real_z = 1.0f0
        end
        ...
        if (is_nearest_neighbour)
            ...
            @inbounds begin
                c00 = Float32(source_arr[x0, y0, z0]) * (1.0f0 - xd) + Float32(source_arr[x1, y0, z0]) * xd
                c10 = Float32(source_arr[x0, y1, z0]) * (1.0f0 - xd) + Float32(source_arr[x1, y1, z0]) * xd
                c01 = Float32(source_arr[x0, y0, z1]) * (1.0f0 - xd) + Float32(source_arr[x1, y0, z1]) * xd
                c11 = Float32(source_arr[x0, y1, z1]) * (1.0f0 - xd) + Float32(source_arr[x1, y1, z1]) * xd
