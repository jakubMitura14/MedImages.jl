...
\begin{abstract}
Targeted radiopharmaceutical therapy (TRT) using Lutetium-177 Prostate-Specific Membrane Antigen ($^{177}$Lu-PSMA) requires highly accurate, patient-specific voxel-level dosimetry. This paper presents a exhaustive programmatic framework using \texttt{DifferentialEquations.jl} and the SciML ecosystem to formulate a Universal Differential Equation (UDE). By embedding a neural network corrector within the radiation transport equations, we rapidly learn the complex transformation from homogeneous kernel convolutions to high-fidelity Monte Carlo (MC) dosemaps. This implementation explicitly incorporates all established physical, biological, and imaging variables for $^{177}$Lu-PSMA, including system-specific calibration, saturable receptor kinetics, and biphasic clearance.
\end{abstract}

\section{Introduction}
Scientific Machine Learning (SciML) offers a path to bridge the gap between fast but inaccurate analytical models and slow gold-standard Monte Carlo (MC) simulations by combining \textbf{Universal Differential Equations (UDEs)} with patient-specific anatomical Computed Tomography (CT) and functional Single-Photon Emission Computed Tomography (SPECT) data \cite{KennedyLugassi2020}.
...
\begin{itemize}
    ...
    \item \textbf{Renal Clearance Scaling:} Excretion rate $k_{10}$ scales with individual estimated Glomerular Filtration Rate (eGFR) \cite{HardiansyahYousefzadehNowshahr2024}.
...
Renal Excretion & $k_{10}$ & Clinical Labs & Scaled dynamically using the individual patient's estimated Glomerular Filtration Rate (eGFR) blood lab results. \\
...
Radiobiology Limits & $\alpha, \beta, \lambda_{repair}$ & Literature & Constants for Sublethal Damage Repair (SLDR) and multi-track interactions. \\ \\ \midrule
...
Hypoxia Modifier & $OER(\mathbf{r})$ & Learned & Neural-discovered spatial oxygen starvation mapping, bounded via a sigmoid activation to physical limits $[1.0, 3.0]$. \\
...
\end{equation}
Furthermore, applying an empirical Recovery Coefficient ($RC(\mathbf{r})$) dynamically at the voxel level is statistically unstable and risks violently amplifying high-frequency Gibbs artifacts at lesion edges; instead, it must be restricted strictly to \textit{bounding Region of Interest (ROI) volumes}.
...
\textbf{2. Transport and Continuous Radiobiology (Continuous Linear-Quadratic (cLQ) model and Hypoxia):}
Instead of a simple linear approximation or discrete post-hoc Biologically Effective Dose (BED) calculations, the ODE natively splits DNA damage into a transient \textbf{Sublethal Damage ($S$)} pool subject to continuous enzymatic repair, and the irreversible accumulation of \textbf{Lethal Lesions ($L$)}. To account for spatial hypoxia, the physical dose rate $\dot{D}(\mathbf{r}, t)$ is dynamically scaled by a neural-learned Oxygen Enhancement Ratio ($OER$).
\begin{align}
    \frac{dTIA}{dt} &= A_{free} + A_{surface} + A_{endo} + A_{lyso} + v_b(\mathbf{r}) \cdot V_{voxel} \cdot \frac{A_{blood}}{V_{blood}} \\
    \dot{D}_{eff}(\mathbf{r}, t) &= \frac{\dot{D}(\mathbf{r}, t)}{OER(\mathbf{r})} \\
    \frac{dS}{dt} &= \dot{D}_{eff}(\mathbf{r}, t) - \lambda_{repair} S(t) \\
    \frac{dL}{dt} &= \underbrace{\alpha \dot{D}_{eff}(\mathbf{r}, t)}_{\text{Single-track kills}} + \underbrace{2\beta \dot{D}_{eff}(\mathbf{r}, t) S(t)}_{\text{Multi-track misrepairs}}
\end{align}
Because unconstrained neural networks can generate mathematically unphysical resistance levels (or negative sensitivities), the learned Hypoxia modifier is algorithmically bounded using a sigmoid ($\sigma$) activation function mapped between known physical limits (where maximum $OER_{max} \approx 3.0$):
\begin{equation}
    OER(\mathbf{r}) = 1.0 + (OER_{max} - 1.0) \cdot \sigma\left(\mathcal{N}_\phi(k_{in}(\mathbf{r}), \nabla\rho(\mathbf{r}), \dots)\right)
\end{equation}
...
\subsection{Group 4: Continuous Radiobiology (continuous Linear-Quadratic (cLQ) model and SciML Hypoxia)}

Tracking the continuous formation of lethal lesions natively:
\begin{equation}
    \frac{dS}{dt} = \dot{D}_{eff}(t) - \lambda_{repair} S(t) \quad \text{and} \quad \frac{dL}{dt} = \alpha \dot{D}_{eff}(t) + 2\beta \dot{D}_{eff}(t) S(t)
\end{equation}

\begin{enumerate}
    \item \textbf{Captured vs. Missing Physics:} By separating damage into Sublethal ($S(t)$) and Lethal ($L(t)$) states, this system captures both the linear (single-track) and quadratic (two-track simultaneous misrepair) mechanisms of the Linear-Quadratic (LQ) model natively over continuous time (Zaider-Minerbo formulation). Furthermore, by embedding a SciML Hypoxia modifier ($OER$), the network explicitly learns the \textbf{Oxygen Enhancement Ratio} spatially, mathematically dampening radiation efficacy in dense, poorly vascularized tumor cores. It misses cell cycle redistribution (resensitization as cells shift from S-phase to G2/M-phase).
    \item \textbf{Numerical Bounds \& Validations:} Both biological states $S(t) \ge 0$ and $L(t) \ge 0$. The OER neural output is strictly constrained to $OER \in [1.0, 3.0]$ preventing negative sensitivities or zero-dose singularities. \textbf{Steady-State Trap:} Mathematically, if inputting a perfectly constant dose rate $\dot{D}_{eff}$, the sublethal pool $S(t)$ must asymptote perfectly to $\frac{\dot{D}_{eff}}{\lambda_{repair}}$. Consequently, the accumulation rate of Lethal Lesions asymptotes to $\frac{dL}{dt} = \alpha \dot{D}_{eff} + \frac{2\beta}{\lambda_{repair}} \dot{D}_{eff}^2$. Integrating this precisely reproduces the analytical Lea-Catcheside G-factor formula for continuous LDR brachytherapy, proving unbroken physical continuity.
    \item \textbf{Probable Ranges \& Distributions:} $S(t)$ forms an \textbf{asymmetric bell curve} over time (rising steeply as early dose is deposited, and decaying as biological washout outpaces physical isotope decay). $L(t)$ is \textbf{strictly monotonically increasing}, representing the cumulative probability of definitive cell death (where survival fraction is $e^{-L}$).
    \item \textbf{Synthetic Data Generation:} To train the Hypoxia neural branch ($\mathcal{N}_\phi$) prior to clinical data, map the spatial distance from the tumor edge. Assign a purely oxic $OER = 1.0$ at the highly vascularized rim. Programmatically force the core geometry to $OER = 3.0$ to simulate a chemically resistant, anoxic necrotic core. Target data can then be generated by evaluating the ODEs, forcing the SciML framework to discover the hidden hypoxia distribution from blunted temporal macroscopic survival curves.
\end{enumerate}

\subsection{Group 5: SciML Radiation Transport \& Neural Corrector}
Deriving absorbed dose natively couples Fast Fourier Transform (FFT) convolutions with unscattered Boltzmann ray-tracing and AI correctors:
\begin{align}
    E_{base,total}(\mathbf{r}) &= TIA \ast (\bar{E}_{\beta} K_{\beta,lung} + \bar{E}_{\gamma} K_{\gamma,water}) \\
    \vec{\Phi}_{ray}(\mathbf{r}) &= \underbrace{\frac{1}{\iiint_V TIA(\mathbf{r}') \, d^3\mathbf{r}'}}_{\text{Activity Normalization}} \iiint_{V} \underbrace{\frac{TIA(\mathbf{r}')}{4\pi |\mathbf{r} - \mathbf{r}'|^2}}_{\text{Inverse-Square}} \underbrace{\exp\left( -\int_{\mathbf{r}'}^{\mathbf{r}} \mu(\mathbf{l}) \, dl \right)}_{\text{Attenuation}} \underbrace{\frac{\mathbf{r} - \mathbf{r}'}{|\mathbf{r} - \mathbf{r}'|}}_{\text{Direction } \mathbf{\hat{\Omega}}} \, d^3\mathbf{r}' \label{eq:ray_tracing} \\
    D_{phys}(\mathbf{r}) &= \frac{E_{base,total}(\mathbf{r})}{m(\mathbf{r})} \cdot \exp\Big(\mathcal{N}_\theta(E_{base,total}, \rho, \nabla\rho, \vec{\Phi}_{ray})\Big)
\end{align}

Equation \ref{eq:ray_tracing} computes the directional vector flux by integrating the unscattered primary photon/electron trajectories over the source volume $V$, accounting for inverse-square falloff and radiological attenuation $\mu$, strictly normalized by the total integrated activity to enforce mathematical superposition. 

To mathematically determine if radiation is colliding with a dense boundary (triggering backscatter) or skimming parallel to it, the Neural Network evaluates the interaction between the density gradient and the incoming flux vector. This geometric collision trigger, $\tau(\mathbf{r})$, is expressed as a dot product:
\begin{equation}
    \tau(\mathbf{r}) = \nabla\rho(\mathbf{r}) \cdot \vec{\Phi}_{ray}(\mathbf{r})
\end{equation}
Because the gradient $\nabla\rho$ points toward increasing density, a heavily \textbf{positive} product ($\tau \gg 0$) indicates that the radiation is moving directly into a high-density interface (like bone), physically cueing the network to reroute energy backward.

\begin{enumerate}
    \item \textbf{Captured vs. Missing Physics:} Base FFT convolutions track isotropic homogeneous energy. The explicit evaluation of the collision trigger $\tau(\mathbf{r})$ allows the network to natively learn directional \textbf{electron backscattering} and heterogeneous attenuation. It misses explicit Bremsstrahlung\textbf{electron backscattering} and heterogenous attenuation. It misses explicit Bremsstrahlung (secondary X-rays from betas decelerating in bone), meaning the NN must learn to hallucinate this background noise from training data alone.
...
Sublethal Damage & $S(t)$ & $\ge 0$ & Asymmetric Bell Curve over time \\
Lethal Lesions & $L(t)$ & $\ge 0$, monotonically asc. & Cumulative probability of cell death \\
Hypoxia Modifier & $OER(\mathbf{r})$ & $[1.0, 3.0]$ & Heavily right-skewed (mostly oxic) \\Density Gradient & $\nabla\rho$ & Locally zero in bulk tissue & Highly leptokurtic Laplace \\
...
\begin{itemize}
    \item \textbf{Teaching Directional Backscatter Awareness:} You feed the network the analytical baseline energy ($E_{base, total}$), local physical density ($\rho$), continuous spatial density boundaries ($\nabla\rho$), and importantly, unscattered ray-traced \textbf{First-Order Directional Vector Flux fields ($\vec{\Phi}_{ray}$)}. By internally evaluating the collision dot product $\tau(\mathbf{r}) = \nabla\rho(\mathbf{r}) \cdot \vec{\Phi}_{ray}(\mathbf{r})$, the network mathematically recognizes perpendicular collisions with dense boundaries ($\tau \gg 0$). It is trained until its output multiplier, $\exp(\mathcal{N}_{\theta})$, successfully maps the baseline dose to your synthetically shifted backscatter target. This explicitly cures "directional blindness," allowing the network to know exactly when and in which geometric direction the backscattered electrons should flow.
...
\noindent\textbf{Thermodynamic Energy Conservation Limit:}
\begin{itemize}
    \item \textbf{The Check:} The SciML constraint limits the neural network to native geometric re-routing by actively enforcing the following physical Loss Function ($\mathcal{L}_{EC}$) penalty during backpropagation:
    \begin{equation}
        \mathcal{L}_{EC} = \lambda_{EC} \left( \iiint_V D_{phys}(\mathbf{r}) \cdot m(\mathbf{r}) \, d^3\mathbf{r} - \iiint_V E_{base,total}(\mathbf{r}) \, d^3\mathbf{r} \right)^2
    \end{equation}
    This strictly enforces the First Law of Thermodynamics, guaranteeing the AI cannot artificially create or destroy radiation energy.
