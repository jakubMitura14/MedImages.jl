...
\begin{itemize}
    ...
    \item \textbf{Imaging Corrections:} Initial activity scaling via the \textbf{Camera Calibration Factor} ($CF$) and resolution recovery via \textbf{Recovery Coefficients} ($RC$). $CF$ is derived from DICOM tags \texttt{(0028,0031)} (Units, e.g., Bq/ml or counts/s/MBq) and quantified using \texttt{(0028,1053)} (Rescale Slope) and \texttt{(0028,1052)} (Rescale Intercept) to convert raw counts to activity \cite{GleisnerChouin2022}. $RC$ values are applied as structure-specific look-up tables based on object size (e.g., from NEMA phantom measurements) to correct for Partial Volume Effects (PVE) \cite{KlettingSchimmel2015}.
\end{itemize}
...
\textbf{What needs to be Learned (Neural Targets):}
\begin{itemize}
    \item \textbf{Heterogeneous Scattering:} The corrector $\mathcal{N}_\theta$ learns residuals caused by density gradients $\nabla \rho$ and mass-energy absorption variations $\mu_{en}/\rho$ \cite{BaileyMarquis2022, MikellKappadath2016}. To facilitate this, the neural network is explicitly designed with three input branches: the instantaneous total activity ($A_{total}$), the local density map ($\rho$), and the spatial density gradient ($\nabla \rho$) computed directly via 3D finite differences.
\end{itemize}
...
The observed SPECT activity $A_{obs}(\mathbf{r}, 0)$ is scaled before integration using the Camera Calibration Factor ($CF$) and the original DICOM Rescale Slope:
\begin{equation}
    A_{true}(\mathbf{r}, 0) = \frac{\left( A_{obs}(\mathbf{r}, 0) \cdot \text{Rescale Slope} \right) \cdot RC(\mathbf{r})}{CF}
\end{equation}
While default variables ($1.0$) are permitted when specific camera calibration factors are unavailable, parameterizing this enables true absolute quantification. Furthermore, while the spatially dependent Recovery Coefficient ($RC(\mathbf{r})$) resolves partial volume effects over small lesions (< 2cm), it is worth noting that this correction primarily dictates accuracy for \textit{per-ROI (Region of Interest)} dosimetry rather than strict per-voxel calculations.
...
function universal_dosimetry_ude!(du, u, p, t)
    ...
    # The neural network explicitly branch-processes Activity, Density, and Density Gradient (∇rho)
    A_total = A_free .+ A_bound
    grad_rho = compute_grad_rho(p.rho)
    dD_phys = compute_neural_dose_rate(A_total, p.rho, grad_rho, p.p_nn, p.st_nn) ./ mass_map
