...
function predict_ude_patch(model, θ, st, sp_raw, den_raw, vol_p)
    p_s = size(sp_raw, 1)
    dev = Lux.gpu_device()
    
    # Aggregation on CPU for scalar fix
    sum_sp = sum(sp_raw)
...
function sliding_window_inference(model, θ, st, spect, ct, vol_p; patch_size=64, stride=32)
    ...
    x_centers = collect(1:stride:nx-patch_size+1); push!(x_centers, nx-patch_size+1)
    y_centers = collect(1:stride:ny-patch_size+1); push!(y_centers, ny-patch_size+1)
    z_centers = collect(1:stride:nz-patch_size+1); push!(z_centers, nz-patch_size+1)
    
    x_centers = unique(x_centers); y_centers = unique(y_centers); z_centers = unique(z_centers)
    ...
    for i in x_centers, j in y_centers, k in z_centers
        ...
        if sum(sp_patch) < 1e-2
            next!(p); continue
        end
        
        den_patch = hu_to_den.(ct[xr, yr, zr])
        
        pred = predict_ude_patch(model, θ, st, sp_patch, den_patch, vol_p)
...
function run_full_body_val()
    ...
    for pat_name in val_cases
        ...
        # Using a simple save logic
        # (Assuming MedImages utils or similar for nifti saving if available, or just use NIfTI package directly)
