...
# Resample
println("\n--- Resample Benchmark (Identity) ---")
b_resample_julia = @benchmark MedImages.resample_to_image($med_im, $med_im, MedImages.Linear_en) samples=5 seconds=10
b_resample_sitk = @benchmark $sitk.Resample($sitk_im, $sitk_im, $sitk.Transform(), $sitk.sitkLinear, 0.0, $sitk_im.GetPixelIDValue()) samples=5 seconds=10

println("Julia Resample (Median): ", median(b_resample_julia).time / 1e6, " ms")
println("SimpleITK Resample (Median): ", median(b_resample_sitk).time / 1e6, " ms")
